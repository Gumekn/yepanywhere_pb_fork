...
const TEST_PROJECT_ID =
  "L1VzZXJzL3BiemhhbmcvRGVza3RvcC_ku6PnoIEveWVwYW55d2hlcmVfcGJfZm9yaw";
...
async function main() {
  ...
  try {
    ...
    if (healthOk && notFoundOk) {
      ...
      console.log("手动测试：");
      console.log(
        "请在浏览器中打开以下链接，验证页面显示友好的错误消息而不是崩溃：",
      );
      console.log(
        `${BASE_URL}/projects/${TEST_PROJECT_ID}/sessions/${TEST_SESSION_ID}`,
      );
      process.exit(0);
    } else {
      console.log("✗ 部分测试失败");
